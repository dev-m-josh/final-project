...
const Hotels = () => {
  ...
  const HotelCard = ({ hotel }: { hotel: HotelType }) => (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 group">
      <div className="relative h-64 overflow-hidden">
        <img
          src={hotel.imageUrl || 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg?auto=compress&cs=tinysrgb&w=800'}
          alt={hotel.name}
  ...
  return (
    <section id="hotels" className={`w-full py-20 bg-gray-50 min-h-screen relative ${(showPopup || showBookingForm || showBookingSuccess) ? 'overflow-hidden' : ''}`}>
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className={`text-center mb-12 transition-all duration-300 ${(showPopup || showBookingForm || showBookingSuccess) ? 'blur-sm' : ''}`}>
          <h1 className="text-4xl font-bold text-gray-900 mb-4">
            Find Your Perfect Hotel
          </h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Discover amazing hotels worldwide and book your perfect room with ease.
            Compare prices, read reviews, and enjoy seamless booking experiences.
          </p>
        </div>
        ...
        {!loading && !error && (
          <>
            {hotels.length > 0 ? (
              <div className={`grid md:grid-cols-2 lg:grid-cols-4 gap-8 transition-all duration-300 ${(showPopup || showBookingForm || showBookingSuccess) ? 'blur-sm' : ''}`}>
                {hotels.map((hotel) => (
                  <HotelCard key={hotel.hotelId} hotel={hotel} />
                ))}
              </div>
            ) : (
              <div className={`text-center py-20 transition-all duration-300 ${(showPopup || showBookingForm || showBookingSuccess) ? 'blur-sm' : ''}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative h-64 overflow-hidden rounded-t-2xl">
              <img
                src={selectedHotel.imageUrl || 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg?auto=compress&cs=tinysrgb&w=800'}
            ...
            <div className="p-8">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className="text-lg font-semibold text-gray-700 ml-2">({selectedHotel.rating})</span>
                </div>
              </div>
